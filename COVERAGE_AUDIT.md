...
| Metric | Before (pre-audit) | After P0-P5 + P1 wiring |
|--------|---------------------|------------------------|
| Tests passing (root `tests/`) | 168 | **173** |
| Tests passing (mission_system) | 127 | **211** |
| **Tests passing (total)** | **295** | **384** |
| Tests failing | 9 | **0** |
| Tests skipped | 36 | **47** |
...
| control_tower references (source) | ~30 | **0** (comments/docstrings only) |
| jeeves_infra source lines | 41,104 | ~12,000 |
| mission_system source lines | 8,616 | ~19,000 |

All P0 phases and P1 wiring work complete. Zero failures, zero collection errors.
The `control_tower` package (deleted Session 14) is fully excised from source code.
47 tests are correctly skipped (43 PostgreSQL, 4 E2E LLM).
...
```
258 collected | 211 passed | 0 failed | 47 skipped | 0 collection errors
```

(Excludes integration tests which need full app stack / PostgreSQL / OpenTelemetry)

**0 Failures** -- `test_memory_imports` fixed by making numpy import lazy in
`EmbeddingService` (moved from module level to `similarity()` method).
...
```
384 passed | 0 failed | 47 skipped | 0 collection errors
```
...
### P1 -- Remaining wiring work (COMPLETED)

| # | Title | Status |
|---|-------|--------|
| 12 | Build out `mission_system/api/governance.py` router | **DONE** (3 endpoints: health, tools, dashboard) |
| 13 | Fix `gateway_chat.py` import | **DONE** (`services.chat_service` → `mission_system.services.chat_service`) |
| 14 | Wire `CommBusServiceStub` into `KernelClient` | **DONE** (`event_emitter.py` uses `_global_client._commbus_stub`) |
| 15 | Full `control_tower` → `kernel_client` rewire | **DONE** (AppState, lifespan, all endpoints, bootstrap, tests) |
| 16 | Make `EmbeddingService` import lazy | **DONE** (numpy moved from module to method level) |
...
1. **Add CI immediately** (P3 #23). Without automated test runs, regressions
   accumulate silently.
2. **Run with PostgreSQL** (`docker compose up -d postgres`) to unlock 43 skipped
   contract and unit tests.
3. **Start P2 coverage work** -- gateway HTTP tests, adapter tests, orchestrator tests.

### Coverage targets

| Milestone | Target | Current |
|-----------|--------|---------|
| P0 + P1 wiring complete | ~30% combined | **ACHIEVED** (384 / ~430 tests pass, 0 failures) |
...
### 6b. Failures (0 remaining)

All failures resolved:

| Failure | Root Cause | Fix |
|---------|-----------|-----|
| `test_memory_imports` | numpy imported at module level | **FIXED**: Moved to `similarity()` method (P1) |
| `test_creates_app_context` | Asserted `control_tower is not None` | **FIXED**: Deprecated field, assertion updated (RCA) |
| 5 kernel_client tests | Proto schema drift | **FIXED**: Removed drifted fields (Phase 2) |
| 3 bootstrap tests | `memory_module` phantom import | **FIXED**: Path fix to `mission_system.memory` (Phase 1+3) |
...
| Tests passing (mission_system) | 127 | 211 | **+84** |
| **Tests passing (total)** | **295** | **384** | **+89** |
| Tests failing | 9 | 0 | **-9** |
...
| Fixed `gateway_chat.py` broken import (`services.chat_service`) | Module-level import error | Chat router now importable |
| Made `EmbeddingService` numpy import lazy | Module-level ImportError | +1 (test_memory_imports) |
| Built `api/governance.py` router (3 endpoints) | Missing governance HTTP layer | Integration tests can run |
| Rewired `app_server.py` control_tower → kernel_client | Dead control_tower references crashed on startup | Server can start without Rust kernel |
| Removed dead resource tracking callback from `bootstrap.py` | Dead code (control_tower always None) | Cleaner bootstrap |
| Rewired `event_emitter.py` CommBus to kernel_client | Dead `from control_tower.ipc.commbus` import | CommBus uses gRPC when available |
